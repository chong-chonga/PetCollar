...
public class UserSettingsRequest {
    ...
    @Pattern(regexp = "^[\\u4e00-\\u9fa5a-zA-Z0-9]{4,16}$",
            message = "用户名必须在 4-16个字符以内，支持中英文、数字", groups = {UpdateTransaction.class})
    @ApiModelProperty(value = "用户名")
    private String username;

    @Pattern(regexp = "^([A-Z]|[a-z]|[0-9]|[~!@#$%^&*()+=|{}':;,\\\\.<>/\\-_?]){6,18}$",
            message = "密码必须在 6-18 位，支持字母、数字、特殊字符",
            groups = {VerificationTransaction.class})
    ...
    public UserProfile createUserProfile() {
        ...
        userProfile.setUserIntroduction(this.userIntroduction);
